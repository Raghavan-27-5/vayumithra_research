...
class Config:
    def __init__(self, **kwargs):
        self.seq_len=336; self.pred_len=1; self.enc_in=51
        self.d_model=128; self.n_heads=4; self.e_layers=2
        self.d_ff=512; self.dropout=0.05; self.activation="gelu"
        self.embed="timeF"; self.freq="h"; self.factor=1
        self.class_strategy="projection"; self.use_norm=True
        self.output_attention=False
        self.__dict__.update(kwargs)

def make_windows(df, fcols, ws_col, start, end, mean, std, seq_len, pred_len):
    mask = (df["datetime"] >= start) & (df["datetime"] < end)
    sub = df[mask]
    vals = (sub[fcols].values.astype(np.float32) - mean) / std
...
train_mean = df[train_mask][feature_cols].mean().values.astype(np.float32)
train_std = (df[train_mask][feature_cols].std().values + 1e-8).astype(np.float32)

t0 = time.time()
tx, ty = make_windows(df, feature_cols, ws_col, df["datetime"].min(), fold["train_end"], train_mean, train_std, 336, 1)
vx, vy = make_windows(df, feature_cols, ws_col, fold["val_start"], fold["val_end"], train_mean, train_std, 336, 1)
tex, tey = make_windows(df, feature_cols, ws_col, fold["test_start"], fold["test_end"], train_mean, train_std, 336, 1)
...
print(f"  Coverage:    {coverage(y_true, p10, p90):.4f} (target ~0.80)", flush=True)
print(f"  Width:       {width(p10, p90):.4f} m/s", flush=True)
print(f"  Winkler:     {winkler(y_true, p10, p90):.4f}", flush=True)
