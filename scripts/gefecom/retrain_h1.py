...
print(f"Windows: train={len(tx)} val={len(vx)}", flush=True)
...
patience_ctr = 0
t_start = time.time()
save_path = f"{SAVE_DIR}/probabilistic_fold1_h1.pt"
...
print(f"Done H1 in {time.time()-t_start:.0f}s (best val={best_val_loss:.4f})", flush=True)
