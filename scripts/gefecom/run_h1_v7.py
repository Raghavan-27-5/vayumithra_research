...
print(f"  Coverage P10-P90: {coverage(y_true, p10, p90):.4f}", flush=True)
print(f"  Coverage P10-P99: {coverage(y_true, p10, p99):.4f}", flush=True)
print(f"  Width P10-P90: {width(p10, p90):.4f} m/s", flush=True)
print(f"  Winkler P10-P90: {winkler(y_true, p10, p90):.4f}", flush=True)
for lbl, q, pq in zip(["P10","P50","P90","P99"], [0.1,0.5,0.9,0.99], [p10,p50,p90,p99]):
    print(f"  {lbl} pinball: {pinball(y_true, pq, q):.4f}", flush=True)
print(f"  Pinball avg: {np.mean([pinball(y_true, p10,.1),pinball(y_true, p50,.5),pinball(y_true, p90,.9),pinball(y_true, p99,.99)]):.4f}", flush=True)
